...
Eine Runde beginnt beim Startspieler. Zuerst werden alle drei Aktivierungsschritte vollständig abgehandelt, danach führt jede Person genau eine Grundaktion aus. Der erste Startspieler wird über die eingespeiste `RandomSource` bestimmt. Nach jeder vollständig abgeschlossenen Runde wandert diese Rolle in der festgelegten Spielerreihenfolge im Uhrzeigersinn weiter. Der Zustand hält die aktuelle Runde und die für die Spielerzahl geltende Höchstzahl fest.

In der Aktivierungsphase entstehen nacheinander genau drei verschiedene Zahlen zwischen 1 und 12. Nach jedem Wurf ermittelt der Core die passenden, in dieser Runde noch nicht aktivierten kontrollierten Gebiete und handelt diese feste Menge vollständig ab, bevor der Startspieler die nächste Zahl würfelt. Jede Zahl wird mit zwei W6 bestimmt: Der erste Würfel wählt eines der Paare `1/2` bis `11/12`; der zweite wählt bei `1–3` die niedrigere und bei `4–6` die höhere Zahl. Doppelte Ergebnisse werden neu gewürfelt. Der Core verwendet dafür kein direktes `Math.random()`, sodass Tests die Würfelfolge genau vorgeben können.

Alle kontrollierten Gebiete mit der aktuellen Aktivierungszahl werden je einmal aktiviert; neutrale Gebiete bleiben aus. Die Spieler sind vom Startspieler aus im Uhrzeigersinn an der Reihe. Hat ein Spieler mehrere offene Gebiete, wählt er deren Reihenfolge selbst durch einzelne Aktivierungsaktionen. Jede Aktion enthält die nötige Symbol- und Zielwahl. Nach einem gültigen Effekt wird das Gebiet als abgehandelt markiert. Erst nach dem dritten abgeschlossenen Aktivierungsschritt beginnt die Aktionsphase.

Die Kandidatenmenge wird jeweils beim Wurf der aktuellen Zahl festgehalten. Erhält ein Gebiet durch ♣ eine zweite Zahl, kann diese einen späteren Wurf derselben Runde auslösen. Bereits abgeschlossene Würfe und die laufende Kandidatenmenge ändern sich dadurch nicht.

| Symbol | Wirkung in Arbeitspaket 2 |
| --- | --- |
| ♦ Karo | Eine Grenze zu einem angrenzenden gegnerischen Gebiet kann für den nächsten Krieg markiert werden. Eine Grenze zu einem neutralen Nachbarn kann von der A4-Basis von bis zu 3 Kästchen bis zur für die Kartengröße gültigen Tiefe geometrisch verschoben werden. Die Aktivierung wartet auf diese Entscheidung. |
...
Nach dem dritten abgeschlossenen Aktivierungsschritt führt jede Person genau eine Grundaktion aus: Auktion oder Krieg. Der aktuelle Spieler kann eine Auktion für ein unmittelbar angrenzendes neutrales Gebiet eröffnen oder mit einem eigenen Gebiet einen angrenzenden Gegner angreifen. Nur wenn beides unmöglich ist, verfällt die Grundaktion. Ein Gebiet, das ein Spieler in der Auktion eines anderen gewinnt, verbraucht seine eigene Grundaktion nicht.
...
Der größte Reichsteil gibt bei zwei Spielern `+10 %`, bei drei Spielern `+15 %` und bei vier bis sechs Spielern `+20 %`. Die geheime Fraktion gibt `+30 %` für die ursprüngliche Kartenfarbe. Jeder Knotenpunkt gibt `+15 %` je unterschiedlichem angrenzenden Gebiet und höchstens `+75 %`; mehrere Knotenpunkte stapeln sich. Ein Frontgebiet mit höchstens `floor(Kartenfläche × 3 %)` Zellen erhält `+20 %` je unterschiedlichem gegnerischen Nachbargebiet. Verbleibender globaler Einfluss zählt am Spielende je `10` Punkte; lokaler Einfluss zählt nicht.
...
Ein normales Gebiet darf pro Runde an einem Krieg beteiligt sein. Ein Großgebiet ab `ceil(Kartenfläche × 6 %)` darf an bis zu zwei Kriegen beteiligt sein, aber höchstens einen davon selbst beginnen; die Einordnung folgt immer seiner aktuellen Rasterfläche. Beide Spieler bestätigen verdeckt höchstens eine verfügbare ♠-Aktivierung. Danach würfelt jeder einmal mit der eingespeisten `RandomSource`. Ein ♠ aus einem Gebiet an der gegnerischen Kampfgrenze gibt +2, sonst +1; jede Festung auf einer Zelle im Verteidigungsgebiet gibt +1. Der Angreifer erhält keinen automatischen Bonus. Der `WarSnapshot` hält die Flächen und gemeinsame Grenze vor dem Kampf fest. Cut-and-Choose-Teile bleiben bis zum Rundenende gesperrt.
...
♦ gegen neutrales Gebiet verwendet ausschließlich eine Basis-Tiefe von 3 auf der A4-Referenzkarte und dieselbe Skalierung. Normale und starke Kriegstiefen sowie ♦-Markierungen gegen Spieler bleiben davon unberührt.
