...
`FileMatchHistoryStore` speichert eine versionierte, unveränderliche JSON-Zusammenfassung pro beendetem Konto-Match in `data/matches/`. Der Room-Snapshot hält nur die kleinen historischen Spitzenwerte für Gebietszahl, Fläche und Grenzgewinn, während alle zählbaren Matchwerte beim Abschluss rein aus dem vollständigen Core-Ereignisprotokoll bestimmt werden. Das Matcharchiv enthält weder vollständige GameStates noch Konto- oder Room-Tokens. `GET /api/me/stats`, `GET /api/me/matches` und `GET /api/me/matches/:matchId` werden jeweils mit der Konto-ID gefiltert; Matchdetails enthalten Aktivitätswerte nur für den angemeldeten Teilnehmer. Beim Restore versucht der RoomManager beendete Konto-Räume idempotent nachzutragen und protokolliert übersprungene Legacy-Daten ohne Namen zuzuordnen.
...
Die Presentation-Schicht im Webclient übersetzt ausschließlich neue, bereits redigierte Core-Ereignisse in kurze Anzeigeeffekte. Sie hält den ersten Snapshot sowie jeden Reconnect als Baseline fest und spielt deshalb weder historische Aktivierungszahlen noch alte Besitzwechsel erneut ab. Aktivierungszahlen, Gebietsimpulse, Symbolbestätigungen, Auktions- und Kampfreveals sowie Besitzwellen verwenden nur die bereits autoritativen Werte und die vorherige Kartenansicht; sie erzeugen keine Aktionen, Zufallswerte oder Spielentscheidungen.
...
Die Browserkarte leitet ihr reines Darstellungsregime ausschließlich aus der autoritativen Phase des `GameReadModel` ab: Bis einschließlich `START_AUCTIONS` bleiben stabile Gebietstöne sichtbar. Ab der ersten regulären Spielphase zeigt jede Karte denselben Besitzmodus mit Spielerfarben und einem dunklen Neutralton. Kartenansichten, Auswahl- und Nachbar-Overlays ändern diese Grundzuordnung nicht. Die Textdarstellung für Strategische Punkte stammt aus einer gemeinsamen UI-Definition und wird in Platzierung, Karte, Details und Regelhilfe wiederverwendet.
...
Nach dem Übergang zu `FINISHED` persistiert der Server erst den kanonischen Room-Snapshot und archiviert dann die idempotente MatchSummary. Falls das separate Archivziel kurzzeitig nicht erreichbar ist, bleibt der abgeschlossene Room maßgeblich; der Start-Restore versucht die fehlende Summary erneut. Ein Rematch erhält immer eine neue Room-ID und damit eine eigene Match-ID.
